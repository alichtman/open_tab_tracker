...
def draw_graph(df: pd.DataFrame, graph_type="line"):
    df['datetime'] = df['datetime'].apply(lambda x: convert_utc_datetime_to_local_formatted_string(x))
    if graph_type == "line":
        fig = px.line(df, x='datetime', y='firefox_tab_count', title='Open tab count over time')
    elif graph_type == "scatter":
        fig = px.scatter(df, x='datetime', y='firefox_tab_count', size='firefox_tab_count', title='Open tab count over time')
